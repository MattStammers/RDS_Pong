...
class Pong(arcade.Window):
  ...
  # Check for paddle boundaries
    ...
    if self.right_player.center_y > SCREEN_HEIGHT - PLAYER_HEIGHT / 2:
     ...
     # movement with
    if self.up_pressed and self.right_player.center_y < SCREEN_HEIGHT - PLAYER_HEIGHT / 2:
      self.right_player.center_y += PLAYER_SPEED

    if self.down_pressed and self.right_player.center_y > PLAYER_HEIGHT / 2:
      self.right_player.center_y -= PLAYER_SPEED

    if self.w_pressed and self.left_player.center_y < SCREEN_HEIGHT - PLAYER_HEIGHT / 2:
      self.left_player.center_y += PLAYER_SPEED

    if self.s_pressed and self.left_player.center_y > PLAYER_HEIGHT / 2:
      self.left_player.center_y -= PLAYER_SPEED
  ...
  def on_draw(self):
    ...
    arcade.draw_text("You can do it!",
                         start_x, start_y,
